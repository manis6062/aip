<?php
	...
	class TradelineOwnerModel extends CI_Model
	{
		...
		{
			$this->db->where($cond);
			$query = $this->db->get("nc_tradelineowner");
		...
		{
			$this->db->select('*');
			$this->db->from('nc_tradelineowner');
			$this->db->where($cond);
		...
		{
			$query = $this->db->get_where('nc_tradeline_owner', array('user_id' => $id));
			if ($query->num_rows() == 0) {
				return 0;
			} else {
				return $query->row();
		...
		function getAll()
		{
			...
			$ref_id = $this->ReferralModel->getReferralIdFromUserId($this->session->userdata(USER_ID));
			$query = $this->db->query("Select t.*, (select count(card_id) from nc_credit_card where to_id = t.to_id AND card_is_delete = 'NO') as totalnumberofcards from nc_tradeline_owner as t where (t.referral_id = '$ref_id' || t.user_id = $userid) and t.to_is_delete = 'NO'");
			if ($query->num_rows() > 0)
				return $query->result();
		...
		function getAllWithPayments()
		{
			if (checkUserType() == 2) {
				...
				$referralid = $this->AffiliateModel->getReferralId($this->session->userdata(USER_ID));
				$query = $this->db->query("Select t.*,
                                                ...
                                                from nc_tradeline_owner as t
                                                where t.to_is_delete = 'NO'
                                                AND  t.referral_id = '$referralid'");
				if ($query->num_rows() > 0) {
					return $query->result();
				} else {
					return 0;
		...
		function getTradelineOwnerDetail($to_id)
		{
			$query = $this->db->get_where('nc_tradeline_owner', array('to_id' => $to_id));
			if ($query->num_rows() == 0) {
				return 0;
			} else {
				return $query->row();
		...
		function deleteTradelineOwner($id)
		{
			$data = array('to_is_delete' => 'YES',);
			$this->db->where('to_id', $id);
			$this->db->update('nc_tradeline_owner', $data);
		...
		function deleteCard($card_id)
		{
			...
			if ($this->db->update('nc_tradeline_owner', $data))
				return TRUE;
			else
				return FALSE;
		}
		...
		{
			$today = date("Y-m-d");
			$data = array('tradeline_owner_name' => $this->input->post('name'),
				'tradeline_owner_address' => ucwords(trim($this->input->post('address'))),
				'tradeline_owner_ssn' => $this->input->post('ssn'),
				'tradeline_owner_email' => trim($this->input->post('email')),
				'tradeline_owner_phone' => $this->input->post('phone'),
				'crtd_dt' => $today,
		...
		function checkavailablecarddetails($code)
		{
			$query = $this->db->query("Select tcd.card_detail_id from
								nc_tradeline_owner_card_details as tcd 
								inner join nc_tradeline_owner as tow 
		...
		function insertInToTradelineowner($userid)
		{
			$today = date("Y-m-d");
			$data = array(
			...
			$this->db->insert('nc_tradeline_owner', $this->security->xss_clean($data));
			$toid = $this->db->insert_id();
			$query = $this->db->query("Select  u.email, concat(t.to_fname, ' ', t.to_lname) as full_name from user as u inner join nc_tradeline_owner as t on t.user_id = u.user_id where u.id= (select user_id from nc_tradeline_owner where to_id = $toid)");
			if ($query->num_rows() > 0) {
				return $query->row();
			} else {
				return 0;
		...
		{
			if ($field == 'to_fname') {
				$this->db->query("UPDATE nc_tradeline_owner, user
                                           SET nc_tradeline_owner.$field = '$value', user.user_name = '$value'
                                           WHERE nc_tradeline_owner.user_id = user.user_id
                                           AND nc_tradeline_owner.user_id = '$id'");
			} else {
				$this->db->query("update nc_tradeline_owner set $field = '$value' where user_id = '$id'");
			}
			...
			if ($field == 'to_email') {
				$this->db->query("UPDATE nc_tradeline_owner, user
                                           SET nc_tradeline_owner.$field = '$value', user.email = '$value'
                                           WHERE nc_tradeline_owner.user_id = user.user_id
                                           AND nc_tradeline_owner.user_id = '$id'");
			} else {
				$this->db->query("update nc_tradeline_owner set $field = '$value' where user_id = '$id'");
			...
			if ($field == 'to_pcon') {
				$this->db->query("UPDATE nc_tradeline_owner, user
                                           SET nc_tradeline_owner.$field = '$value', user.phone = '$value'
                                           WHERE nc_tradeline_owner.user_id = user.user_id
                                           AND nc_tradeline_owner.user_id = '$id'");
			} else {
				$this->db->query("update nc_tradeline_owner set $field = '$value' where user_id = '$id'");
			...
			if ($field == 'to_address') {
				$this->db->query("UPDATE nc_tradeline_owner, user
                                           SET nc_tradeline_owner.$field = '$value', user.address = '$value'
                                           WHERE nc_tradeline_owner.user_id = user.user_id
                                           AND nc_tradeline_owner.user_id = '$id'");
			} else {
				$this->db->query("update nc_tradeline_owner set $field = '$value' where user_id = '$id'");
		...
		{
			$query = $this->db->query("SELECT ct.id, ct.type FROM card_type ct ORDER BY id ASC");
			if ($query->num_rows() > 1)
				return $query->result();
			else {
				return null;
		...
		function tradelineownerWithPayment($to_id)
		{
