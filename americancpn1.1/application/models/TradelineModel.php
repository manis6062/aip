...
class TradelineModel extends CI_Model
{
    ...
    {
        return $this->db->query("SELECT t.id, t.user_id, concat(p.first_name , ' ' , p.last_name) user, t.card_type, t.bank, t.url, t.phone, t.lmt, t.balance, t.open, t.statement, t.charge, t.payment, t.max, t.status, t.note FROM tradeline t LEFT JOIN profile p ON p.user_id = t.user_id")->result();
    }
    ...
    public function getAll()
    {
        ...
        if (checkUserType() == 1 || checkUserType() == 2) {
            ...
            $query = $this->db->query("SELECT
										...
										s.scheme_limit,
										s.scheme_charge,
										tow.tradeline_owner_name,
										tocd.card_number,
										tocd.expiration_date,
										tocd.card_limit,
										tocd.card_selling_limit,
										tow.tradeline_owner_address,
										tow.tradeline_owner_email,
										tow.tradeline_owner_ssn										
										FROM
										nc_tradeline_schemes AS s
										INNER JOIN nc_tradeline_owner_card_details AS tocd ON s.card_detail_id = tocd.card_detail_id
										INNER JOIN nc_tradeline_owner AS tow ON tocd.owner_id = tow.owner_id
										WHERE
    ...
    {
        $query = $this->db->query("Select owner_id from nc_tradeline_owner where rcode = '$code'");
        if ($query->num_rows() > 0) {
    ...
    function insertCardDetails($code, $cardtype, $bank, $cardnumber, $exp_date, $credit_limit, $credit_selling_limit)
    {
        ...
        $this->db->insert('nc_tradeline_owner_card_details', $this->security->xss_clean($data));
        $query = $this->db->query("Select * from nc_tradeline_owner_card_details where owner_id = '$owner_id'");
        if ($query->num_rows() > 0) {
            return $query->result();
        }
        return 0;
    ...
    {
        $query = $this->db->query("Select * from nc_tradeline_owner_card_details where card_detail_id = $card_details_id");
        return $query->row();
    }
    ...
    {
        $query = $this->db->query("SELECT
									IFNULL((tocd.card_selling_limit-(select IFNULL(SUM(scheme_limit),0) from nc_tradeline_schemes WHERE card_detail_id = tocd.card_detail_id)),0) as availablelimit from nc_tradeline_owner_card_details as tocd
    ...
    function generatescheme($amount, $charge, $card_detail_id)
    {
        ...
        $this->db->insert('nc_tradeline_schemes', $this->security->xss_clean($data));
    ...
    function getSingleUsers($id)
    {
        ...
        $query = $this->db->query("SELECT * from nc_tradeline_owner where to_id = $id");
        //var_dump($query);
        if ($query->num_rows() > 0)
            return $query->row();
    ...
    function getSingleUsersForUserId($id)
    {
        ...
        //$query = $this->db->query("SELECT c.*, (SELECT state FROM states  WHERE id = c.state) as state1 FROM nc_client c WHERE c.id =$id");
        $query = $this->db->query("SELECT * from nc_tradeline_owner where user_id = $id");
        //var_dump($query);
        if ($query->num_rows() > 0)
            return $query->row();
    ...
    function checkcodeanduseravailability($code)
    {
        ...
        $query = $this->db->get_where('nc_request_for_signup', array('rcode' => $code, 'request_to_type' => 'tradeline'));
        if ($query->num_rows() > 0) {
            $row = $query->row();
    ...
    function getReferralInfoForClient($ref_user)
    {
    ...
    function insertTradeline2($username, $password)
    {

        $today = date("Y-m-d H:i:s");
        $priviledge = "";
        $val = $this->db->query("SELECT nc_auth_master.auth_id FROM nc_auth_master WHERE nc_auth_master.module IN (SELECT modules FROM nc_tradelineowner_module_priviledge) ORDER BY nc_auth_master.auth_id");
        if ($val->num_rows() > 0) {
            $result = $val->result();
            $i = 1;
    ...
    function insertTradeline()
    {
        ...
        $this->db->insert('tradeline', $this->security->xss_clean($data));
        $cardid = $this->db->insert_id();
        return $cardid;
    }

    function insertInToTradelineTable($userid)
    {
        $today = date("Y-m-d H:i:s");
        $data = array(
        ...
        $this->db->insert('nc_tradeline_owner', $this->security->xss_clean($data));
        $tradeline = $this->db->insert_id();
        $query = $this->db->query("Select  u.email, concat(t.to_fname, ' ', t.to_lname) as full_name from user as u inner join nc_tradeline_owner as t on t.user_id = u.user_id where u.id= (select user_id from nc_tradeline_owner where to_id = $tradeline)");
        if ($query->num_rows() > 0) {
            return $query->row();
        } else {
            return 0;
    ...
    function tradelineGetAll()
    {
        $this->db->select('*');
        $this->db->from('nc_request_for_signup');
        $this->db->order_by("request_id", "desc");
        $this->db->where("request_to_type", "tradeline");
        $query = $this->db->get();
    ...
    function insert_tradeline($email, $rcode)
    {
        $today = date("Y-m-d");
        $type = '';
        if ($this->session->userdata(ADMIN_AUTH_TYPE) != 'affiliate' && $this->session->userdata(ADMIN_AUTH_TYPE) != 'referral' && $this->session->userdata(ADMIN_AUTH_TYPE) != 'client') {
            $type = 'referral';
        } elseif ($this->session->userdata(ADMIN_AUTH_TYPE) == 'affiliate' || $this->session->userdata(ADMIN_AUTH_TYPE) == 'referral') {
            $type = 'tradeline';
        }
