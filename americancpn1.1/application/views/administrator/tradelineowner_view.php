...
<script type="text/javascript">
	function pageredirect(data, id) {
		window.location = "<?php echo base_url().'administrator/tradelineowner/view/';?>" + id + '#' + data;
	}
</script>
		...
		<div class="box span12">
			...
			<div class="box-content">
				...
				<?php if ($usertype != 1 && $usertype != 3) { ?>
					<a href="<?php echo site_url(ADMIN_PATH . 'tradelineowner/add'); ?>" class="btn btn-primary">Add New
						Tradeline Owner</a>
				<?php } ?>
				<table class="table table-striped table-bordered bootstrap-datatable datatable">
					...
					<tr>
						<th>S.No.</th>
						<th>Tradeline Owner<?php echo '&nbsp;&nbsp;&nbsp'; ?><img
						...
						if ($ownerList != 0 && count($ownerList) > 0) {
							...
							foreach ($ownerList as $values) {
								...
								<tr class="item">
									...
									<td>
										<a href="<?php echo site_url(ADMIN_PATH . 'tradelineowner/view/' . $values->user_id); ?>"><?php echo $values->to_fname . ' ' . $values->to_mname . ' ' . $values->to_lname; ?></a>
									</td>

									<td><a title="Card Details"
										   href="<?php echo base_url() . 'administrator/tradelineowner/carddetails/' . $values->to_id . '/' . $values->to_fname . '_' . $values->to_lname; ?>"><?php echo $values->totalnumberofcards; ?></a>
									</td>
									...
									<?php if ($usertype != 1) { ?>
										<td class="action" style="width:100px">
											...
											<?php if (($values->totalnumberofcards) == 0) { ?>
												<a href="<?php echo site_url(ADMIN_PATH . 'tradelineowner/deleteLineowner/' . $values->to_id); ?>"
												   onclick="javascript:return confirm('Are you sure you want to delete this Tradeline Owner?')"
												   title="Click to delete"> <img
														src="<?php echo base_url(); ?>/style/img/delete.png"
														alt="Delete Tradeline Owner Details"></a>
											<?php } else { ?>
												<a onclick="javascript:return confirm('Delete the Cards Before Deleting Tradeline Owner')"
												   title="Click to delete"> <img
														src="<?php echo base_url(); ?>/style/img/delete.png"
														alt="Delete Tradeline Owner Details"></a>
											<?php } ?>
