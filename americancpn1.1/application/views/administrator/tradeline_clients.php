                        ...
                        <table class="table table-striped table-bordered bootstrap-datatable datatable">
                            <thead>
                                <tr>
                                    ...
                                    <th>No. of Tradelines<?php echo '&nbsp;&nbsp;&nbsp'; ?><img src="<?php echo base_url(); ?>/style/img/sort.png"></th>
                                    <!-- <th>Action</th> -->
...
<button type="button" data-toggle="popover<?php echo $key;?>" data-placement="top" title="Tradeline Lists" data-trigger="focus" 
...
<!-- <td class="center sorting_1">
  <?php 
  if ($values -> countcard == 0){ ?>
 <a onclick="return confirm("Make Sure Before You Delete This Record")" href="<?php echo site_url(ADMIN_PATH . 'tradelineowner/deleteSingleAction1/' . $val->card_sell_id.'/'.$val->card_id); ?>" class="btn btn-danger" >Remove</a>
  <?php } ?>
 <a class="btn btn-primary" href="<?php echo site_url(ADMIN_PATH . 'tradelineowner/notAddedClients1/' . $val->card_sell_id.'/'.$val->card_id . '/' . $val->client_id); ?>"  onclick="return confirm('Are you sure you do not want to add ?')">Not Added</a>
...
<button type="button" data-toggle="popover1<?php echo $key;?>" data-placement="top" title="Tradeline Lists" data-trigger="focus" 
    data-content="<ul><?php foreach($cardinfopro as $values)
    {
    if($val->id == $values->client_id){?>
    <li><?php echo $values->card_name ;if($values->card_type){echo ' | '.$values->card_type; }?></li>
          <?php } }
    ?>
  ...
  <input type="button" class="btn btn-primary"  value="Verify"  />
   <?php } ?>
                    ...
                    <!-- <?php if ($val -> countcard == 0) { ?>
                                <a class="btn btn-danger" href="<?php echo site_url(ADMIN_PATH . 'tradelineowner/deleteSingleAction1/' . $val->card_sell_id.'/'.$val->card_id . '/' . $val->id); ?>"  onclick="return confirm('Are you sure you do not want to remove  ?');">Remove</a></td> -->
                            <!-- <?php } ?>    -->         
