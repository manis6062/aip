...
class Poll extends CI_Controller {
    ...
    function show($page = '') {
        ...
        $data['pollList'] = $this->PollModel->getAllPoll($config['per_page'], $offset);
    ...
    function deleteAction($id, $offset) {
        $details = $this->PollDetailModel->getPhotoDetails($id);
        $path = POLL_IMAGE_PATH;
        foreach($details as $data){
            @unlink($path.$data->image);
        }
        if ($this->PollModel->deletePoll($id)) {
            if($this->PollDetailModel->deletePollDetail($id)){
    ...
    function viewDetail($id) {
        $data['pollData'] = $this->PollModel->getData($id);
        $data['pollList'] = $this->PollModel->getDetail($id);
