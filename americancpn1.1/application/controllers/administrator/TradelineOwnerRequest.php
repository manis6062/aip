...
class TradelineOwnerRequest extends CI_Controller {
    ...
    function show($id, $page = '') {
        ...
        if (in_array('referral_view', $this -> allowed)) {
            $data['list'] = $this -> TradelineModel -> tradelineGetAll();
            ...
            $data['main_content'] = ADMIN_PATH . "tradelineowner_requestforsignup";
    ...
    function deleteAction($id) {
        ...
        if (in_array('referral_view', $this -> allowed)) {
            ...
            if ($this -> TradelineModel -> delete($id[0])) {
        ...
        redirect(ADMIN_PATH . "tradelineowner_request");
    }
        ...
        if (in_array('referral_view', $this -> allowed)) {
        	...
        	foreach($this->input->post('email') as $value){
        		if(!$this->UserModel->checkemailregistered($value)){
                    ...
                    $this->EmailerModel->RequestForSignupFromReferral($email ,$rcode);
...
//						$msg.= "Here is the link for Tradeline Registration: <a href='".base_url()."tradeline/".$this->session->userdata(ADMIN_AUTH_NAMEUSER)."/".$rcode."'>Sign Up</a>";
//						$msg.= '<!doctype html>
//									<html>
//									<head>
//									<meta charset="utf-8">
...
//
//						$msg.= '<h3 style="padding:10px; margin:0px; color:#fff;"><img src="'.base_url().'frontend/images/logo.png" height="35px" style="vertical-align:middle;margin-right:15px;">American CPN</h3>';
//						$msg.='</div>
...
//
//
//					}elseif($this->session->userdata(ADMIN_AUTH_TYPE)!='affiliate' || $this->session->userdata(ADMIN_AUTH_TYPE)!='referral' || $this->session->userdata(ADMIN_AUTH_TYPE)!='client'){
...
//						$msg.= '<p style="font-size:13px;">Here is the link for Referral Registration: <a href="'.base_url().'register/referral/'.$rcode.'">Sign Up</a></p>';
//
...
//	                $m->setMessageFromString('', $message);
        ...
        if (in_array('referral_view', $this->allowed)) {
            ...
            else //success  
            {
                foreach ($_POST['msg'] as $id){
                    if ($this->TradelineModel->delete($id)) {
                    	$flag = TRUE;
                    } else {
						$flag = FALSE;
                    }      
	...
	function generatercode(){
		...
		if($this->TradelineModel->checkcode($rcode)){
			$this->generatercode();
		}else{
			return $rcode;			
		}
