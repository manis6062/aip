...
class Album extends CI_Controller {
    ...
    function show($page = '') {
        if (in_array('album_view', $this->allowed)) {
            $data['photoList'] = $this->AlbumModel->getAll();
    ...
    function deleteAction($id, $offset) {
        if (in_array('album_delete', $this->allowed)) {
            ...
            if(!$this->GalleryModel->checkGallery($id)){
                ...
                $this->session->set_flashdata("su_message", "This album has gallery images so please first delete all the images of this album.");
            }
        } else {
    ...
    function update() {
        if (in_array('album_update', $this->allowed)) {
            if ($this->form_validation->run('album_update') == FALSE) {
                ...
                //files validations
                    $this->AlbumModel->update($this->input->post('album_id'));
