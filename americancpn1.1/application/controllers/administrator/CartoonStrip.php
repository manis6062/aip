...
class Cartoonstrip extends CI_Controller {
    ...
    function show($page = '') {
        ...
        $data['cartoonstoryList'] = $this->CartoonStripModel->getAll('all');
    ...
    function deleteAction($id, $offset) {
        $details = $this->CartoonStripModel->getPhotoDetails($id);
        if ($this->CartoonStripModel->deletePhoto($id)) {
    ...
    function add() {
        if ($this->form_validation->run('cartoon_add') == FALSE) {
            ...
            if ($photo) {
                $this->CartoonStripModel->insert($ph, $pic);
    ...
    function update($offset) {
        if ($this->form_validation->run('cartoon_update') == FALSE) {
            ...
            if ($photo) {
                ...
                $this->CartoonStripModel->update($this->input->post('id'), $ph);
    ...
    function changeStatus($id, $value, $offset) {
        ...
        if ($this->CartoonStripModel->updateStatus($id, $stat)) {
