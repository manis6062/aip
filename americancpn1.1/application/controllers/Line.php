...
//  
		...
		{
			$email = $this->LineModel->checkcodeanduseravailability($code);
			$refid = $this->LineModel->getreferrerInfoForClient($ref_user);
			if ($email) {
				$data['refinfo'] = $refid;
				$data['ref_user'] = $ref_user;
				$data['email'] = $email;
				$data['code'] = $code;
				$data['title'] = "Lineowner Sign Up";
				$data['states'] = $this->AffiliateModel->getStates();
				$data['main_content'] = 'line_registration';
...
// if(empty($_POST['to_transunion'] || $_POST['to_experion'] || $_POST['to_equifax'])){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$username = $this->LineModel->getGenerateUsername();
				$password = $this->LineModel->generatePassword();

				$user_id = $this->LineModel->insertLine($username, $password);
				if ($user_id) {
					$line = $this->LineModel->insertInToLineTable($user_id);
					if (!empty($line)) {
						...
						$msg .= '<p style="font-size:13px;">Hello <strong>' . $line->to_fname . '</strong> You are registered successfully in americancpn website. Your Login credentials for the site is:</p>';
		...
		function success()
		{
			...
			$data['title'] = "Lineowner Register Success";
			$data['main_content'] = 'line_registration_success';
