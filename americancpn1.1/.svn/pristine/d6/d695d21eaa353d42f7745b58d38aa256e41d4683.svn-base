...
class Affiliate extends CI_Controller
{
    ...
    function emails()
    {
        if (in_array('referrer_view', $this->allowed)) {
            ...
            if ($user_type == 'affiliate' || $user_type == 'referrer') {
                ...
                $data['clients'] = $this->EmailsModel->getallclients($user_id);
                $data['type'] = 'referrer';
    ...
    function sendEmails()
    {
        ...
        if (in_array('referrer_view', $this->allowed)) {
            if ($this->input->post('user_type') == 'super-admin') {
                $superadmin = $this->EmailsModel->getSuperAdmin();
                $superemail = '';
                $superid = '';
                ...
                foreach ($superadmin as $key => $value) {
                    ...
                    if ($flag) {
                        if ($this->EmailsModel->insertEmails('super-admin', 'Success', $superid)) {
                            $flag = TRUE;
                        } else {
                            $flag = FALSE;
                        }
                ...
                $receiver = implode(',', $this->EmailsModel->getIds($emailto));
                foreach ($emailsto as $key => $value) {
                    $this->EmailerModel->sendEmailsToAdminWithUserTypeClient($value);
                }
                if ($flag) {
                    if ($this->EmailsModel->insertEmails('client', 'Success', $receiver)) {
                        $flag = TRUE;
                    } else {
                        $flag = FALSE;
                    }
                ...
                foreach ($emailsto as $key => $value) {
                    ...
                    if ($flag) {
                        if ($this->EmailsModel->insertEmails('referrer', 'Success', $receiver)) {
                            $flag = TRUE;
                        } else {
                            $flag = FALSE;
                        }
    ...
    function emailsreceiverdelete($email_id)
    {
        $email_id = $email_id[0];
        if (in_array('referrer_view', $this->allowed)) {
            if ($this->EmailsModel->deleteEmails($email_id)) {
