...
class Publication extends CI_Controller {
    ...
    function show($page = '') {
        ...
        $config['total_rows'] = $this -> PublicationModel -> countAll($cond);
        ...
        $data['start'] = $page;
        $data['usersTypes'] = $this -> PublicationModel -> getAllPaginate($cond, $config['per_page'], $offset);
    ...
    function deleteAction($user_id, $offset) {
        if (in_array('user_update', $this -> allowed)) {
            ...
            $details = $this -> PublicationModel -> getAdminDetails($user_id);

            if ($this -> PublicationModel -> deleteuser($user_id)) {
    ...
    function add() {
        ...
        if (in_array('publication_add', $this -> allowed)) {
            if ($this -> form_validation -> run('publication_add') == FALSE) {
                ...
                if ($photo) {
                    $this -> PublicationModel -> insert($ph);
        ...
        if (in_array('publication_update', $this -> allowed)) {
            if ($this -> form_validation -> run('publication_edit') == FALSE) {
                ...
                if ($photo) {
                    ...
                    $this -> PublicationModel -> update($this -> input -> post('publication_id'), $ph);
    ...
    function updateOrder($id, $order, $low_high, $offset) {
        if ($low_high <= 1) {
            $this -> PublicationModel -> changehigherorder($id, $order);
        } else {
            $this -> PublicationModel -> changelowerorder($id, $order);
        }
    ...
    function changeStatus($id, $value, $offset) {
        ...
        if ($this -> PublicationModel -> updateStatus($id, $stat)) {
