...
class Cartoon extends CI_Controller {
    ...
    function show($page = '') {
        ...
        $data['cartoonList'] = $this->CartoonModel->getAll('all', $config['per_page'], $offset);
    ...
    function deleteAction($id, $offset) {
        $details = $this->CartoonModel->getPhotoDetails($id);
        $title = $details->title;
        if ($this->CartoonModel->deleteCartoon($id)) {
    ...
    function deleteStripAction($id, $cid, $offset) {
        $details = $this->CartoonStripModel->getCartoonStrips($id);
        print_r($details);
        $title = $details->cartoon;
        if ($this->CartoonStripModel->deleteCartoonStrip($id)) {
            $path = CARTOON_IMAGE_PATH;
            $folder = str_replace(' ', '_', $title);
            $dir = $path.$folder;
    ...
    function addAction() {
        ...
        $data['parentCartoon'] = $this->CartoonModel->parentCartoon();
    ...
    function add() {
        if ($this->form_validation->run('cartoon_add') == FALSE) {
            ...
            if ($photo) {
                $this->CartoonModel->insert($ph, $pic);
    ...
    function update($offset) {
        if ($this->form_validation->run('cartoon_update') == FALSE) {
            ...
            $titledata = $this->CartoonModel->getSingleCartoon($this->input->post('id'));
            ...
            if ($photo) {
                ...
                $this->CartoonModel->update($this->input->post('id'), $phfeat, $picbanner);
    ...
    function insertPoll() {
        if ($this->form_validation->run('poll_add') == FALSE) {
            ...
            if ($photo) {
                ...
                $pollid = $this->PollModel->insert($cartoon_id);
                $i = 1;
                        ...
                        if ($j==$i) {
                            if($_FILES["files"]["name"][$key]!=""){
                                ...
                                if (move_uploaded_file($tname, $path.$name)) {
                                    $ph = $name;
                                    $this->PollDetailModel->insert($pollid, $values, $ph, 0);
                                }
                            }else{
                                $ph = "";
                                    if ($values != "") {
    ...
    function updatePoll($id, $offset) {
        $masterauth = new AuthModel();
        $data['error'] = $this->errors;
        $data['pollRecord'] = $this->PollModel->getSinglePoll($id);
...
//            $config['height'] = 251;
    ...
    function changeStatus($id, $value, $offset) {
        ...
        if ($this->CartoonModel->updateStatus($id, $stat)) {
    ...
    function changeEditorsPick($id, $value, $offset) {
        ...
        if ($this->CartoonModel->updateEditorPick($id, $stat)) {
    ...
    function uploadImage($id, $title){
        if(isset($_POST) && $_SERVER['REQUEST_METHOD'] == "POST"){
            //$titledata = $this->CartoonModel->getSingleCartoon($this->input->post('cartoon'));
            ...
            if($ext=='jpg' || $ext=='jpeg' || $ext=='gif' || $ext=='png'){
                if(move_uploaded_file(strip_tags($_FILES['upload_file']['tmp_name']), $file_location)){
                    $this->CartoonStripModel->insert($file_name, $id);
                    echo $file_id;
                }else{
                    echo 'system_error';
                }
    ...
    function updateOrder($cartoon_id,$id, $order, $low_high, $offset) {
        if ($low_high <= 1) {
            $this->CartoonModel->changehigherorder($cartoon_id, $order);
        } else {
            $this->CartoonModel->changelowerorder($cartoon_id, $order);
        }
    ...
    function updateOrderCartoon($cartoon_id, $order, $low_high, $offset) {
        if ($low_high <= 1) {
            $this->CartoonModel->changehigherorderCartoon($cartoon_id, $order);
        } else {
            $this->CartoonModel->changelowerorderCartoon($cartoon_id, $order);
        }
        $this->session->set_flashdata("su_message", "Order changed successfully");
        redirect(ADMIN_PATH . "cartoon/show/$offset");
    }
    function changeBannerShow($id, $value, $offset) {
        ...
        if ($this->CartoonModel->updateBannerShow($id, $stat)) {
    ...
    function changeRecentShow($id, $value, $offset) {
        ...
        if ($this->CartoonModel->updateRecentShow($id, $stat)) {
