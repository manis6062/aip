...
<?php
	...
	if ($this->session->userdata(ADMIN_AUTH_TYPE) == 'affiliate' && $this->session->userdata(ADMIN_AUTH_TYPE) == 'referrer') {
		...
		$allowed = $this->session->userdata(AUTHS);
	}
?>
<!-- topbar starts -->
<div>
				...
				<ul class="dropdown-menu" role="menu">
					<?php
						...
						if ($this->session->userdata(ADMIN_AUTH_ROLE) == '5') {
							?>
							<li>
								<a href="<?php echo site_url(ADMIN_PATH . 'referrer/view/' . $this->session->userdata(USER_ID)); ?>">Profile</a>
							</li>
							<li></li>
							<li>
								<a href="<?php echo site_url(ADMIN_PATH . 'user/changePassword/' . $this->session->userdata(USER_ID)); ?>">Change
						...
						if ($this->session->userdata(ADMIN_AUTH_ROLE) == '6') {
							?>
							<li>
								<a href="<?php echo site_url(ADMIN_PATH . 'lineowner/view/' . $this->session->userdata(USER_ID)); ?>">Profile</a>
							</li>
							<li></li>
							<li>
								<a href="<?php echo site_url(ADMIN_PATH . 'user/changePassword/' . $this->session->userdata(USER_ID)); ?>">Change
						...
						if ($this->session->userdata(ADMIN_AUTH_TYPE) == 'client') {
							$referrerStatus = $CI->ClientModel->checkClientPaymentSatus($this->session->userdata(USER_ID));
							{
								{
