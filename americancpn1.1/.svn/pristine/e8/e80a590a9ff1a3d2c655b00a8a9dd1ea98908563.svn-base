...
class Ipnlistener extends CI_Controller {
	...
	public function paymentProcess() {
		...
		if ($_SERVER['REQUEST_METHOD'] == "POST") {
			$val = $_POST['custom'];
			list($aid, $fee) = explode('-', $val);
			
			if($fee == 'fee'){
				$admininfo = $this->UserModel->getAdminPaypalInfo();
				
				$receiver_email = $_POST['receiver_email'];
				if ($receiver_email == $admininfo->paypal_account) {
						// Check number 2 ------------------------------------------------------------------------------------------------------------
						if ($_POST['payment_status'] == "Completed") {
							...
							$updatepayment = $this -> AffiliateModel -> paypalPyamentInsert($fee, $aid, $paytitle, $pamount, $payer_email, $txn_id, $pstatus);
	
							// if ($updatepayment!='') {
	
								$subject = 'CPN referrer Partner Registration Payment';
								...
								$msg .= 'Payment Successfully completed. Thank you!';
								...
								$m -> addTo('contact <'.$updatepayment->affiliate_email.'>');
				...
				if ($receiver_email == $admininfo->paypal_account) {
					...
					if ($payment_amount == $admininfo->referrer_reg_charge) {
						...
						if ($_POST['payment_status'] == "Completed") {
							...
							$updatepayment = $this -> AffiliateModel -> paypalPyamentInsert('', $aid, $paytitle, $pamount, $payer_email, $txn_id, $pstatus);
							...
							// if ($updatepayment!='') {
								...
								$msg .= 'Registration Payment Successfully completed. Please login <a href="' . base_url() . 'admin"> Click Here</a>';
