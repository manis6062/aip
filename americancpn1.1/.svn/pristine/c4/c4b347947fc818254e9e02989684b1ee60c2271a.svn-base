...
class Gallery extends CI_Controller {
    ...
    function show($id, $page = '') {
        if (in_array('gallery_view', $this -> allowed)) {
            $album_title = $this -> AlbumModel -> getDetails($id);
            $data['photoList'] = $this -> GalleryModel -> getAllByAlbum($id);
    ...
    function deleteAction($id) {
        ...
        if (in_array('gallery_delete', $this -> allowed)) {
            $album_id = $this -> GalleryModel -> getImageDetail($id['0']);
            ...
            if ($this -> GalleryModel -> delete($id['0'])) {
    ...
    function add() {
        if (in_array('gallery_add', $this -> allowed)) {
            if ($this -> form_validation -> run('gallery_add') == FALSE) {
                ...
                $folder = $this -> AlbumModel -> getDetails($this -> input -> post('album_id')) -> album_title;
        ...
        if (in_array('gallery_update', $this -> allowed)) {
            if ($this -> form_validation -> run('gallery_add') == FALSE) {
                ...
                if ($photo) {
                    ...
                    $this -> GalleryModel -> update($this -> input -> post('id'), $ph);
    ...
    function updateAction($gallery_id) {
        if (in_array('gallery_view', $this -> allowed)) {
            $album = $this -> GalleryModel -> getImageDetail($gallery_id['0']);
            $album_title = $this -> AlbumModel -> getDetails($album -> album_id);
            $data['photoList'] = $this -> GalleryModel -> getAllByAlbum($album -> album_id);
            $data['allowed'] = $this -> allowed;
            $data['albumname'] = $album_title -> album_title;
    ...
    function changeStatus($id, $value, $offset) {
        ...
        if ($this -> AlbumModel -> updateStatus($id, $stat)) {
    ...
    function remove_checked(){
        if (in_array('gallery_delete', $this->allowed)) {
            ...
            if ($this->form_validation->run() == FALSE)   
            {
                ...
                redirect(ADMIN_PATH . "gallery/".$_POST['album']);   
            }  
            else //success  
            {
                foreach ($_POST['msg'] as $id){
                    $details = $this->GalleryModel->getPhotoDetails($id);
                    if ($this->GalleryModel->delete($id)) {
                        ...
                        $this->session->set_flashdata("su_message", "<font color=\"#FF0000\">The Selected Gallery Image Can't Be Deleted.</font>");
                    }      
                }      
                //redirect to inbox                                       
            } 
            ...
            $this->session->set_flashdata("su_message", "You Have No Permission To Delete This Gallery Image");
        }
