...
class Character extends CI_Controller {
    ...
    function show($page = '') {
        ...
        $data['characterList'] = $this->CharacterModel->getAll('all', $config['per_page'], $offset);
    ...
    function deleteAction($id, $offset) {
        $details = $this->CharacterModel->getPhotoDetails($id);
        if ($this->CharacterModel->deletePhoto($id)) {
    ...
    function update($offset) {
        if ($this->form_validation->run('character_update') == FALSE) {
            ...
            if ($photo) {
                ...
                $this->CharacterModel->update($this->input->post('id'), $ph);
    ...
    function updateAction($id, $offset) {
        ...
        $masterauth = new AuthModel();
        $data['error'] = $this->errors;
        $data['photoRecord'] = $this->CharacterModel->getPhotoDetails($id);
    ...
    function changeStatus($id, $value, $offset) {
        ...
        if ($this->CharacterModel->updateStatus($id, $stat)) {
