...
class Writer extends CI_Controller {
    ...
    function show($page = '') {
        if (in_array('writer_view', $this -> allowed)) {
            $cond = array();
            $config['total_rows'] = $this -> WriterModel -> countAll($cond);
            ...
            $data['start'] = $page;
            $data['usersTypes'] = $this -> WriterModel -> getAllPaginate($cond, $config['per_page'], $offset);
    ...
    function deleteAction($user_id) {
        ...
        if (in_array('writer_delete', $this -> allowed)) {
            $details = $this -> WriterModel -> getAdminDetails($user_id);
            if ($this -> WriterModel -> deleteuser($user_id)) {
                $path = TEAM_PATH;
    ...
    function checkBeforeDelete($user_id) {
        if ($user_id == $this -> session -> userdata(USER_ID)) {
            ...
            $details = $this ->UserModel -> getAdminDetails($user_id);
        ...
        if (in_array('writer_update', $this -> allowed)) {
            if ($this -> form_validation -> run('writer_add') == FALSE) {
                ...
                if ($photo) {
                    ...
                    $this -> WriterModel -> update($this -> input -> post('writer_id'), $ph);
    ...
    function updateAction($user_id) {
        $masterauth = new AuthModel();
        $data['error'] = $this -> errors;
        $data['usersTypes'] = $this -> WriterModel -> getAdminDetails($user_id);
    ...
    function updateOrder($id, $order, $low_high, $category) {
        if ($low_high <= 1) {
            $this -> WriterModel -> changehigherorder($id, $order, $category);
        } else {
            $this -> WriterModel -> changelowerorder($id, $order, $category);
        }
